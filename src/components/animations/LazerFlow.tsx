...
#define R_H 150.0
#define R_V 450.0
#define FLARE_HEIGHT 18.0
#define FLARE_AMOUNT 8.0
#define FLARE_EXP 2.0
#define TOP_FADE_START 0.35
#define TOP_FADE_EXP 0.7
...
#define HFOG_Y_RADIUS 150.0
#define HFOG_Y_SOFT 250.0
...
#define DITHER_STRENGTH 1.0
    ...
    float maxY = R_V * uVLenFactor;
    float span=smoothstep(-3.0,0.0,y)*(1.0-smoothstep(maxY-6.0,maxY,y));
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    float radialFade = 1.0 - smoothstep(0.0, 0.8, length(uvc) / 450.0);
...
export const LaserFlow: React.FC<Props> = ({
  ...
  const resolveColor = (colorStr: string) => {
    if (colorStr.startsWith('var(')) {
      if (typeof window !== 'undefined') {
        const variableName = colorStr.match(/var\((--[^)]+)\)/)?.[1];
        if (variableName) {
          const computedColor = getComputedStyle(document.documentElement).getPropertyValue(variableName).trim();
          if (computedColor) return hexToRGB(computedColor);
        }
      }
    }
    return hexToRGB(colorStr || '#FFFFFF');
  };
  ...
  useEffect(() => {
    ...
    uniforms.uBeamXFrac.value = horizontalBeamOffset + 0.15;
    uniforms.uBeamYFrac.value = verticalBeamOffset;
    uniforms.uFlowSpeed.value = flowSpeed;
    uniforms.uVLenFactor.value = verticalSizing * 4.0;
    ...
    const { r, g, b } = resolveColor(color || '#FFFFFF');
    ...
    <div
      ...
      onMouseMove={(e) => {
        ...
        if (el) {
          ...
          el.style.setProperty('--my', `${y}px`); // Removed vertical centering offset to match mouse exactly
        }
        // Also update standard shader mouse uniforms if needed, although the effect binds to canvas events separately
      }}
      onMouseLeave={() => {
        const el = revealImgRef.current;
        if (el) {
